...
export interface IClient {
     ...
     * @return OK
     */
    getCategory(categoryId: string): Promise<CategoryEntity>;
...
export class Client implements IClient {
    ...
    protected processGetCategory(response: Response): Promise<CategoryEntity> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = CategoryEntity.fromJS(resultData200);
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
     ...
     */
    getSkill(skillId: string): Promise<SkillModel> {
        let url_ = this.baseUrl + "/api/skill/{skillId}";
        if (skillId === undefined || skillId === null)
            throw new Error("The parameter 'skillId' must be defined.");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetSkill(_response);
        });
    }

    protected processGetSkill(response: Response): Promise<SkillModel> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = SkillModel.fromJS(resultData200);
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
